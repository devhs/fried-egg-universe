# Azure VM 빠른 배포 (Ubuntu 22.04 LTS x64 · 무료 도메인 DuckDNS + HTTPS · GitHub)
...
3. **무료 도메인 — DuckDNS** (인증서 발급용, 도메인 따로 살 필요 없음):
   - https://www.duckdns.org 로그인(Google/GitHub) → 서브도메인 add (예: `myegg` → `myegg.duckdns.org`) → **token** 복사.
   - A 레코드 연결은 2단계 스크립트가 자동으로 합니다(`DUCKDNS_DOMAIN`/`DUCKDNS_TOKEN` 전달 시).
   - 이미 내 도메인이 있으면 DuckDNS 대신 그 도메인의 A 레코드를 VM 공용 IP로 지정하고 `APP_DOMAIN`을 쓰세요.
...
./deploy-azure.sh
# → 끝나면 https://myegg.duckdns.org 로 접속
...
```
브라우저에서 `https://myegg.duckdns.org` (또는 본인 도메인) 접속.

---

## 도메인 없이 공용 IP로만 (HTTP)
2단계에서 `DUCKDNS_*`, `APP_DOMAIN`, `CERTBOT_EMAIL` 을 모두 빼고 실행하면 `http://<VM-공용IP>` 로 바로 뜹니다.
