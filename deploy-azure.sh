...
#   ./deploy-azure.sh
#
# APP_DOMAIN / CERTBOT_EMAIL 를 비우면 HTTPS 없이 공용 IP로 HTTP만 구성합니다.
# 재실행하면 git pull + 의존성 갱신 + 무중단 reload 로 재배포됩니다.
#
# 주의: NSG(방화벽) 80/443 개방, 22는 내 IP만 — 이건 Azure Portal/CLI에서 따로 설정하세요.
#       (이 스크립트는 VM 내부 설치/구동만 담당합니다. DEPLOY-AZURE.md 2번 참고)
set -euo pipefail
...
say "1/8 시스템 업데이트 + 필수 패키지 (Ubuntu apt)"
...
if [ -n "$APP_DOMAIN" ]; then
  say "7/8 HTTPS 발급 (certbot) — 도메인 DNS가 이 서버 공용 IP를 가리켜야 성공합니다"
  sudo apt install -y certbot python3-certbot-nginx
  sudo certbot --nginx -d "$APP_DOMAIN" --non-interactive --agree-tos -m "$CERTBOT_EMAIL" --redirect
else
  say "7/8 HTTPS 건너뜀 (APP_DOMAIN 미설정 — 공용 IP로 HTTP 접속)"
fi
